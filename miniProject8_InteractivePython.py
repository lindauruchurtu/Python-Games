# Mini-project # 8 - Full Asteroids (RiceRocks)
#--------------------------------------------
# TO DO: Add explosions!
# -------------------------------------------
...
class Ship:
    ...
    def update(self):
        ...
        if self.thrust == True:
            self.vel[0] += 0.2*forward[0]
            self.vel[1] += 0.2*forward[1]
...
def process_sprite_group(canvas,a_set_of_sprites):
    copy_of_a_set_of_sprites = set(a_set_of_sprites)
    for sprite in copy_of_a_set_of_sprites: 
        sprite.draw(canvas)
        if sprite.update() == True:
            a_set_of_sprites.remove(sprite)
            

def group_collide(other_object,group):
    # create copy of group to iterate over 
    copy_of_group = set(group)
    # Check for collison
    for sprite in copy_of_group:
        if sprite.collide(other_object) == True:
            group.remove(sprite) # remove from legit group
            return True
...
def group_group_collide(group,other_group):
    ...
    copy_of_group = set(group)
    for sprite in copy_of_group:
        if group_collide(sprite, other_group) == True:
...
def draw(canvas):
    ...
    if group_collide(my_ship,rock_group) == True:
        lives = lives - 1
    ...
    if lives == 0:
        ...
        soundtrack.pause()
        soundtrack.rewind()
        my_ship.reset_position() # set ship at centre
        timer.stop()
...
def rock_spawner():
    ...
    while not safe_distance:
        ...
        if dist( my_ship.get_position(), rock_pos ) > my_ship.get_radius() + 50:
            safe_distance = True
